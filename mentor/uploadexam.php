...
<body>
  ...
  <div style='width:100%;height:40px;background-color:#f5f5f5'>
   <a href="adminlogin.php" style='margin-left:1260px'>&nbsp LOG OUT </a>
      <button type="button" id="btn1"  style='width:200px;FONT-WEIGHT:bold;margin-left:500px;background:linear-gradient(to right, #709BFE,#45BBFE);margin-top:3px;height:50px;focus:none;outline:none;border:none '>Upload Exam Forms</button>
	...
	<table cellspacing=0 cellpadding=10 style='background-color:#f5f5f5'>
		...
		<tr  style='background-color:#efefef'><td width=200px><a href='uploadexam.php'><img src='admin images/examup.png' style='width:20px;height:20px'/>&nbsp Upload Exam forms</td></td>
   ...
   <script>
		...
		$(document).ready(function()
		{
		   ...
		   {
		   var id=$(this).attr("id");
		     $.post("formdelete.php",{ids:id},function(data)
			 {
			   $("#blue").html(data).show();
			   return false;
			 });
		   ...
		   $("#meclick").click(function()
		   {
			 ...
			 $.post("getformupload.php",{course:course,degree:degree,date:date,last:last},function(data)
			 {
			   $("#su").html(data).show();
			   return false;
			 });
	...
	<div style='margin-top:-400px;float:right; width:80%'>
	     
		 
		 
		 <div id="green" style='display:none;width:100%;height:100%;margin-top:-160px'>
	  <form action="" method="POST" >
		  <input type="text" id="date" value='<?php echo  date("Y-m-d") ; ?>' name='date' disabled style='margin-left:360px'/><br><br>
		  <select name='select1' id="select1" required style="width:220px;margin-left:120px;height:30px;font-weight:bold;" >
		  ...
		  <input type='date' id='last' name='last'><span style='font-weight:bold'> (Last date)</span>
...
<br><br>
	...
	<div id="blue" style='display:none;width:100%;height:100%;margin-left:-100px;margin-top:-200px'>
	    <?php
