...
<body onload="myFunction()">
 ...
 <div style='width:100%;height:40px;background-color:#f5f5f5'>
   <a href="adminlogin.php" style='margin-left:1260px'>&nbsp LOG OUT </a>
      <button type="button" id="btn1"  style='width:200px;FONT-WEIGHT:bold;margin-left:500px;background:linear-gradient(to right, #709BFE,#45BBFE);margin-top:3px;height:50px;focus:none;outline:none;border:none '>Upload Forms</button>
	...
	<table cellspacing=0 cellpadding=10 style='background-color:#f5f5f5'>
		...
		<tr style='background-color:#efefef'><td width=200px><a href='eval.php'><img src='admin images/eval.png' style='width:20px;height:20px'/>&nbsp Upload Re-eval <span style='margin-left:35px'>  Forms</span></td></td>
   ...
   <script>
		...
		$(document).ready(function()
		{
		   ...
		   {
		   var id=$(this).attr("id");
		     $.post("evaldelete.php",{ids:id},function(data)
			 {
			   $("#blue").html(data).show();
			   return false;
			 });
		   ...
		   $("#meclick").click(function()
		   {
			 ...
			 var session=document.getElementById("select3").value;
			
			 $.post("geteval.php",{course:course,degree:degree,session:session},function(data)
			 {
			   $("#su").html(data).show();
			   return false;
			 });
	...
	<div style='margin-top:-500px;float:right; width:80%'>
	     
		 
		 
		 <div id="green" style='display:none;width:100%;height:100%;margin-top:5px'>
	  <form action="" method="POST" >
		 <input type="text" id="date" value='<?php echo  date("d/m/Y") ; ?>' name='date' disabled style='margin-left:360px'/><br><br>
		   <select name='select3' id="select3" required style="width:220px;margin-left:120px;height:30px;font-weight:bold;" >
		  ...
		  <select name='select1' id="select1" required style="width:220px;margin-left:1px;height:30px;font-weight:bold;" >
	...
	<div id="blue" style='display:none;width:100%;height:100%;'>
	    <?php
		      $con=mysqli_connect("localhost","root","","examport");
			   $r=mysqli_query($con,"select * from evaluation");
			   $Sno=1;
