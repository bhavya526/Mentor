...
<header id="header" class="fixed-top" style=';;margin-top:3px;height:5px;background-color:rgba(0,0,0,0)'>
  ...
  <div style='width:100%;height:40px;background-color:#f5f5f5'>
	...
	<table cellspacing=0 cellpadding=10 style='background-color:#f5f5f5'>
		...
		<tr style='background-color:#efefef' ><td width=200px style=''><a href='examlist.php'><img src='admin images/arrow.png' style='margin-left:40px;width:10px;height:10px'/>&nbsp List</td></td>
...
if(isset($_POST["geTY"]))
	  {
            $geki=$_POST['deg11'];
			 ...
			 echo "<tr style='border-bottom:3pt solid black;color:black'>
					   ...
					   <td align='center' style='font-weight:bold'>Application No.</td></tr>";
					   
			 $con=mysqli_connect("localhost","root","","examport");
			 $r=mysqli_query($con,"select * from examform where coursess = '$geki' and sems='$kiel' and sessions='$uki' and Paymentstatus='Paid'");
		    ...
		    while($row=mysqli_fetch_array($r))
			{
			   ...
			   echo "<tr style='border-bottom:1pt solid black;color:black'>
					 ...
					 <td align='center'>$row[29]</td>
					 <td align='center'>$row[50]</td></tr>";
					 $sno++;
	   ...
	   {
	         $con=mysqli_connect("localhost","root","","examport");
			 $r=mysqli_query($con,"select * from examform where Paymentstatus='Paid'");
